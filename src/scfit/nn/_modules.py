...
class FunctionalModule(torch.nn.Module):
    """Wrap a plain callable as an ``nn.Module`` (so a lambda can live inside a ``Sequential``)."""
...
class MLP(torch.nn.Module):
    """Configurable multilayer perceptron — optional batch/layer-norm, dropout, and per-layer activations."""
    ...
    def forward(
        ...
        """Run the MLP over the last dim, preserving the leading (batch) dims."""
...
class Resnet1d(torch.nn.Module):
    """Conditioned residual MLP: a stack of blocks, each adding a projection of ``cond`` into the residual."""
    ...
    def forward(
        ...
        """Transport ``x`` through the conditioned residual stack, preserving leading dims."""
        original_shape = x.shape[:-1]
        x = x.reshape(-1, x.shape[-1])
    ...
    def output_dim(
        ...
        """Feature width of the output."""
        return self._output_dim


class AdaLNZero1d(torch.nn.Module):
    """DiT-style adaLN-zero trunk — residual blocks whose LayerNorm is modulated by a conditioning vector.

    Modulation comes from a **zero-initialized** projection of ``cond``.
    ...
    def __init__(
        ...
        *,
        ...
        blocks = []
        for _block_id in range(self._num_blocks):
    ...
    def output_dim(self) -> int:
        """Feature width of the output (constant across blocks)."""
        return self._dim

    def forward(self, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        """Apply the adaLN-zero blocks, modulating each block's LayerNorm by ``cond``."""
        original_shape = x.shape[:-1]
        x = x.reshape(-1, x.shape[-1])
        cond = cond.reshape(-1, cond.shape[-1])
