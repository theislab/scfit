...
:class:`~scfit.training._objective.Objective`, and its ``training_step`` just asks the objective for a
...
from scfit.training._objective import Objective
...
class TrainingModule(pl.LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        """Ask the objective for ``(loss, logs)``, log every metric, and return the loss."""
    ...
    def configure_optimizers(self) -> torch.optim.Optimizer:
        """Build the optimizer over ``model.parameters()`` at the configured learning rate."""
