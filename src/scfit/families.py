"""The model-**family** registry + discovery — the ``model.family`` dispatch.

A *family* is a training paradigm (flow matching, contrastive foundation). Concretely it is just a callable
``(recipe) -> FamilyBuilder`` — and a **builder class is exactly that** (constructing it *is* building), so a
family needs no wrapper class: the entry point points straight at the builder.
...
class FamilyBuilder(Protocol):
    ...
    * ``trainer_overrides: dict`` — extra ``Trainer`` kwargs the family's own eval needs (e.g.
    ...
    module: Any  # a lightning.LightningModule (e.g. scfit.training.TrainingModule)
    datamodule: Any  # a lightning.LightningDataModule (or None + explicit dataloaders)
    callbacks: list

    def save(self, path: Any) -> None:
        """Persist the builder's model bundle (config spec + weights) to ``path``."""
        ...
